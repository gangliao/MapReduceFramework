...
public:
  ...
  std::unordered_map<std::string, std::function<std::shared_ptr<BaseMapper>()>>
      mappers_;
  std::unordered_map<std::string, std::function<std::shared_ptr<BaseReducer>()>>
      reducers_;
...
std::shared_ptr<BaseMapper> TaskFactory::get_mapper(
...
std::shared_ptr<BaseReducer> TaskFactory::get_reducer(
    const std::string& user_id) {
...
bool register_tasks(
  ...
  return factory.mappers_.insert(std::make_pair(user_id, generate_mapper))
             .second &&
         factory.reducers_.insert(std::make_pair(user_id, generate_reducer))
             .second;
}
